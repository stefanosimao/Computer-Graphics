...
public:
    ...
    Hit intersect(Ray ray){
        ...
        if(D<=radius){
			...
			float gamma = atan2(hit.normal.z, hit.normal.x);
			// float theta = asin(hit.normal.y/radius);
			float theta = atan2(hit.normal.y, hit.normal.z);

			hit.uv.s = (gamma + M_PI) / (2 * M_PI);
			// hit.uv.t = (theta + M_PI / 2) / M_PI;
			hit.uv.t = (theta + M_PI) / (2 * M_PI);
		}
		else{
            hit.hit = false;
...
public:
	...
	Hit intersect(Ray ray){
		...
		float podoto = glm::dot(point - ray.origin, normal);
		float ddotn = glm::dot(ray.direction, normal);

		float t = podoto / ddotn;
...
glm::vec3 PhongModel(glm::vec3 point, glm::vec3 normal, glm::vec2 uv, glm::vec3 view_direction, Material material){
	...
	for(int light_num = 0; light_num < lights.size(); light_num++){
		...
		float lightD = glm::distance(lights[light_num]->position, point);
		float att = 1 / (1.0 + 0.01 * lightD + 0.0005 * pow(lightD, 2));
...
void sceneDefinition (){
	...
	green_diffuse.ambient = glm::vec3(0.002f, 0.15f, 0.002f);
	green_diffuse.diffuse = glm::vec3(0.4f, 0.8f, 0.4f);
	green_diffuse.specular = glm::vec3(0);
	green_diffuse.shininess = 0.0;

	Material red_specular;
	red_specular.diffuse = glm::vec3(0.9f, 0.2f, 0.2f);
	red_specular.ambient = glm::vec3(0.15f, 0.002f, 0.002f);
	red_specular.specular = glm::vec3(0.5);
	red_specular.shininess = 10.0;

	Material blue_specular;
	blue_specular.ambient = glm::vec3(0.002f, 0.002f, 0.15f);
	blue_specular.diffuse = glm::vec3(0.3f, 0.3f, 0.9f);
	...
	Material green_wall;
	green_wall.ambient = glm::vec3(0.07f, 0.09f, 0.07f);
	green_wall.diffuse = glm::vec3(0.15f, 0.55f, 0.1f);
	objects.push_back(new Plane(glm::vec3(0, 0, 30), glm::vec3(0, 0, -1), green_wall));

	Material red_wall;
	red_wall.ambient = glm::vec3(0.07f, 0.05f, 0.04f);
	red_wall.diffuse = glm::vec3(0.65f, 0.4f, 0.4f);
	objects.push_back(new Plane(glm::vec3(-15, 0, 0), glm::vec3(1, 0, 0), red_wall));

	Material blue_wall;
	blue_wall.ambient = glm::vec3(0.09f, 0.07f, 0.06f);
	blue_wall.diffuse = glm::vec3(0.40f, 0.42f, 0.8f);
	objects.push_back(new Plane(glm::vec3(15, 0, 0), glm::vec3(-1, 0, 0), blue_wall));

	Material grey_wall;
	grey_wall.ambient = glm::vec3(0.08f, 0.08f, 0.08f);
	grey_wall.diffuse = glm::vec3(0.9f, 0.9f, 0.9f);
	objects.push_back(new Plane(glm::vec3(0, -3, 0), glm::vec3(0, 1, 0), grey_wall));
...
glm::vec3 toneMapping(glm::vec3 intensity){
	...
	tonemapped.x = min(pow(0.9 * pow(intensity.x, 0.99), 1 / 2.0), 1.0);
	tonemapped.y = min(pow(0.9 * pow(intensity.y, 0.99), 1 / 2.0), 1.0);
	tonemapped.z = min(pow(0.9 * pow(intensity.z, 0.99), 1 / 2.0), 1.0);
